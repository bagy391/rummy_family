...
export default function TableCenter({
  ...
  useEffect(() => {
    ...
    if (myHand.length === prevHand.length + 1) {
      ...
      if (newSizes.length === 0) {
        ...
        const currentVal = newSizes[lastIdx] ?? 0;
        if (currentVal < 5) {
          newSizes[lastIdx] = currentVal + 1;
        } else {
          newSizes.push(1);
        }
      }
    ...
    if (myHand.length === prevHand.length - 1) {
      ...
      if (removedIdx !== -1) {
        ...
        if (targetRowIdx !== -1) {
          const newSizes = [...rowSizes];
          const currentVal = newSizes[targetRowIdx] ?? 0;
          newSizes[targetRowIdx] = currentVal - 1;
          if ((newSizes[targetRowIdx] ?? 0) === 0) {
  ...
  const handleDragInternal = (_event: any, info: any) => {
    ...
    // Convert page coordinates to viewport/client coordinates to match getBoundingClientRect()
  ...
  const handleDragEndInternal = () => {
    if (activeDragIdx !== null && hoveredSlotIdx !== null && hoveredSlotIdx !== activeDragIdx) {
      ...
      if (hoveredSlotIdx === myHand.length) {
        targetRowIdx = rowSizes.length;
      } else {
        accum = 0;
        for (let i = 0; i < rowSizes.length; i++) {
          accum += rowSizes[i] ?? 0;
          if (hoveredSlotIdx < accum) {
            targetRowIdx = i;
            break;
          }
        }
      ...
      if (sourceRowIdx !== -1 && targetRowIdx !== -1) {
        const newSizes = [...rowSizes];
        const newHand = [...myHand];
        const draggedCard = myHand[activeDragIdx];

        if (draggedCard) {
          isLocalReorderRef.current = true;

          // Case A: Dropping onto "+ New Group" target
          if (targetRowIdx === rowSizes.length) {
            const currentVal = newSizes[sourceRowIdx] ?? 0;
            newSizes[sourceRowIdx] = currentVal - 1;
            newSizes.push(1);

            newHand.splice(activeDragIdx, 1);
            newHand.push(draggedCard);
          }
          // Case B: Dropping onto an existing row
          else {
            const targetSize = rowSizes[targetRowIdx] ?? 0;

            if (sourceRowIdx === targetRowIdx) {
              // Same row reordering: just move the card
              newHand.splice(activeDragIdx, 1);
              newHand.splice(hoveredSlotIdx, 0, draggedCard);
            } else {
              // Different row
              if (targetSize >= 5) {
                // Target row is full (has 5 cards). We perform a swap/displacement!
                let targetRowStart = 0;
                for (let i = 0; i < targetRowIdx; i++) {
                  targetRowStart += rowSizes[i] ?? 0;
                }
                const targetRowLastIdx = targetRowStart + targetSize - 1;
                const displacedCard = myHand[targetRowLastIdx];

                if (displacedCard) {
                  // Swap them in the flat array
                  newHand[targetRowLastIdx] = draggedCard;
                  newHand[activeDragIdx] = displacedCard;

                  // Now, slide the dragged card to hoveredSlotIdx within the target row
                  const rowCards = newHand.slice(targetRowStart, targetRowStart + targetSize);
                  rowCards.pop(); // Remove draggedCard from the end of sliced row
                  const relInsertIdx = hoveredSlotIdx - targetRowStart;
                  rowCards.splice(relInsertIdx, 0, draggedCard);

                  newHand.splice(targetRowStart, targetSize, ...rowCards);
                }
              } else {
                // Target row has space (< 5 cards)
                const currentVal = newSizes[sourceRowIdx] ?? 0;
                newSizes[sourceRowIdx] = currentVal - 1;
                newSizes[targetRowIdx] = targetSize + 1;
  ...
  rowSizes.forEach((size) => {
    cardRows.push(myHand.slice(startIdx, startIdx + size));
    startIdx += size;
  });
  if (cardRows.length === 0 && myHand.length > 0) {
    for (let i = 0; i < myHand.length; i += 4) {
      cardRows.push(myHand.slice(i, i + 4));
    }
  }
      ...
      >
        <span
          className={`text-[15px] sm:text-[17px] font-black ${
            isMyTurn ? "text-[var(--color-gold)]" : "text-white/60"
          }`}
        >
          {isMyTurn
            ? hasDrawnThisTurn
        ...
        <div className="flex-1 flex flex-col justify-center items-center">
          <div ref={containerRef} className="w-full h-full rounded-2xl bg-[#2D5265]/40 border-2 border-[#C67035]/80 relative flex flex-col p-2 sm:p-4 shadow-inner backdrop-blur-sm justify-center items-center min-h-[320px] sm:min-h-[460px]">
            {isSpectator ? (
              ...
              <div className="flex flex-col gap-y-2 sm:gap-y-3.5 w-full items-center overflow-visible">
                {cardRows.map((rowCards, rowIdx) => (
                  <div
                    key={rowIdx}
                    className="flex items-center justify-center w-full max-w-[340px] sm:max-w-[420px] overflow-visible"
                  >
                    {rowCards.map((card, cardIdxInRow) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className={`w-[64px] h-[96px] sm:w-[88px] sm:h-[132px] rounded-lg bg-white border-2 text-black cursor-pointer shrink-0 relative select-none ${
                            cardIdxInRow > 0 ? "-ml-8 sm:-ml-12" : ""
                          } ${
                            isSelected
                        ...
                        >
                          <div className="w-full h-full flex flex-col justify-between p-1 sm:p-1.5 relative overflow-hidden rounded-lg">
                            {isCardJoker ? (
                              ...
                              <>
                                ...
                                <div className="flex flex-col items-center leading-none self-start select-none z-10 relative">
                                  <span className={`text-[20px] sm:text-[26px] font-black leading-none ${suitColor} tracking-tighter`}>
                                    {rank}
                                  </span>
                                  <span className={`text-[12px] sm:text-[15px] font-black leading-none ${suitColor} mt-0.5`}>
                                    {symbol}
                                  </span>
                                </div>

                                <div className={`text-[18px] sm:text-[24px] text-center font-bold self-center leading-none opacity-20 scale-125 select-none ${suitColor}`}>
                                  {symbol}
                                </div>

                                <div className="flex flex-col items-center leading-none scale-y-[-1] scale-x-[-1] self-end select-none z-10 relative">
                                  <span className={`text-[20px] sm:text-[26px] font-black leading-none ${suitColor} tracking-tighter`}>
                                    {rank}
                                  </span>
                                  <span className={`text-[12px] sm:text-[15px] font-black leading-none ${suitColor} mt-0.5`}>
                ...
                {!isSpectator && rowSizes.length < 5 && (
                  <div
                    ...
                    className={`w-[200px] h-[32px] sm:h-[36px] border-2 border-dashed rounded-xl flex items-center justify-center text-[11px] font-black uppercase tracking-wider transition-colors mt-2 ${
                      hoveredSlotIdx === myHand.length
        ...
        <div className="w-[105px] sm:w-[130px] md:w-[150px] lg:w-[170px] shrink-0 bg-[#2D5265]/20 border border-white/5 rounded-2xl p-1.5 sm:p-2.5 md:p-3.5 lg:p-4.5 pb-3 sm:pb-5 md:pb-7 lg:pb-9 flex flex-col gap-2 sm:gap-3.5 md:gap-4.5 lg:gap-5.5 items-center justify-start backdrop-blur-sm shadow-md transition-all duration-200">
          ...
          <div className="w-full flex flex-col gap-1.5 shrink-0 select-none">
            <button
              ...
              className={`w-full py-1.5 md:py-2 lg:py-2.5 rounded-lg border text-[9px] md:text-[10px] lg:text-[11px] font-black uppercase tracking-wider flex items-center justify-center gap-1 md:gap-1.5 lg:gap-2 transition-all ${
                isMenuExpanded
              ...
              <div className="w-full flex flex-col gap-2 md:gap-2.5 bg-black/40 p-1.5 md:p-2 rounded-xl border border-white/5 overflow-hidden animate-scale-in">
                ...
                <div className="flex items-center gap-0.5 md:gap-1.5 w-full justify-between pt-0.5 md:pt-1 border-t border-white/5">
                  ...
                  <button
                    ...
                    className={`p-1 md:p-1.5 rounded flex items-center justify-center ${
                      isConfirmingQuit ? "bg-red-600 text-white" : "text-red-400 hover:bg-red-500/10"
                    }`}
                    title="Quit Room"
                  >
